...
public class DBApp {
	...
	public void createTable(String strTableName, String strClusteringKeyColumn,
		...
		// surroud the whole method with try catch to catch any exception and re-throw
		// it as DBAppException
		try {
			if (strTableName == null || strClusteringKeyColumn == null || htblColNameType == null
					|| htblColNameMin == null || htblColNameMax == null)
				throw new DBAppException("One of the parameters is null!");
			if (strTableName.equals("") || strClusteringKeyColumn.equals("") || htblColNameType.isEmpty()
					|| htblColNameMin.isEmpty() || htblColNameMax.isEmpty())
				throw new DBAppException("One of the parameters is empty!");
			// check if table already exists
			if (listofCreatedTables.contains(strTableName))
				throw new DBAppException(
						"Table already exists! If you want to\n reset or Create it again," + " please call ."
								+ "DELETETableDependencies(TableName) method first then call createTable() method");

			validation(htblColNameType, htblColNameMin, htblColNameMax); // validation method checks if the type of
																			// table's columns is one of the 4 types
																			// specified in the description,validation
																			// also checks if any column does not have
																			// maxVal or minVal , hence throws exception

			for (String colName : htblColNameMin.keySet()) // converts all min and max column values to lowercase
															// (hosain)
...
//        	i++;
	...
	public void insertIntoTable(String strTableName, Hashtable<String, Object> htblColNameValue) throws DBAppException {
		...
		try {
			...
			Methods.convert_strings_To_Lower_Case(htblColNameValue); // convert all string records in hashtable to lower
																		// case
...
//            tblToInsertInto.validateColType(colType(strColName));
			...
			// insert the non given column values with NULL (other than the clustering key,
			// if any)
			if (vecValues.size() < tblToInsertInto.getVecColumns().size()) { // not all columns are inserted
				for (Column c : tblToInsertInto.getVecColumns()) {
					...
					if (!htblColNameValue.containsKey(colName))
						vecValues.add(tblToInsertInto.getColumnEquivalentIndex(colName), NULL); // insert null in the
																								// correct index of the
																								// column
				}
			}
	...
	public void updateTable(String strTableName, String strClusteringKeyValue,
		...
		// surround the whole method with try catch to catch any exception and re-throw
		// it as DBAppException
		try {
			...
			Methods.check_strings_are_Alphabitical(htblColNameValue); // check if all string records inside the
																		// hashtable are alphabitical
			Methods.convert_strings_To_Lower_Case(htblColNameValue); // convert all string records in hashtable to lower
																		// case
...
//            tblToUpdate.validateColType(colType(strColName));
			...
			while (iterate.hasNext()) {
				...
				if (tblToUpdate.loadPage(index).isEmpty()) {
					...
					// new File(this.getClass().getResource("/resources/tables/" + strTableName +
					// "/pages/" + pagetodelete + ".ser").toURI()).delete();
					// delete from disk
				} else
					index = iterate.nextIndex();
			}
	...
	public void createIndex(String strTableName, String[] strarrColName) throws DBAppException {
		try {
			...
			Octree tree = new Octree(new OctPoint(min_x, min_y, min_z), new OctPoint(max_x, max_y, max_z),
					this.MaximumEntriesinOctreeNode);
...
//    		}
	...
	public static void OperationSignatureDisplay(OpType operation, Table table,
		...
		case DELETE:
			...
			if (htblColNameVal.size() > 0) {
				...
				while (en2.hasMoreElements())
					cols2[k2++] = (String) en2.nextElement();
		...
		default:
			System.out.println("No enough or Wrong Info Given");

		}
	}

	public static void OperationSignatureDisplay(Table table, Hashtable<String, String> htblColNameType,
			OpType operation) {
		if (!OUTPUT_RESULTS)
			return;
		...
		case CREATE:
			...
			for (int i = 0; i < cols4.length; i++) {
				...
				if (!table.getStrClusteringKeyColumn().equals(cols4[i]))
					output4.append(
							(i == cols4.length - 1) ? htblColNameType.getOrDefault(cols4[i], "Null") + "\n );\n\n"
									: htblColNameType.getOrDefault(cols4[i], "Null") + ",\n");
				else
					output4.append((i == cols4.length - 1)
	...
	public static void OperationSignatureDisplay(OpType operation, Table table,
			Hashtable<String, Object> htblColNameVal, String clusterKey) {
		if (!OUTPUT_RESULTS)
			return;
		...
		if (operation == OpType.UPDATE) {
			...
			while (en3.hasMoreElements())
				cols3[k3++] = (String) en3.nextElement();
	...
	public void readConfig() throws DBAppException {
		 ...
		 * extract data needed for setting the instance variables of the DBApp Class
		...
		try {
			...
			while (line != null) {
				...
				if (!set.contains(values[0]))
					listofCreatedTables.add(values[0]);
				set.add(values[0]);
				line = br.readLine();
			}
	...
	// createIndex helpers
	private static void writeIndexInMetadata(String tableName, String[] columnNames, String indexName)
			throws IOException {// puts the index name in the metadata file instead of null in each column in
								// columnNames
	...
	private void fillTreeFromTable(Octree tree, Table table, String[] strarrColName)
	...
	// select helpers
	private Set<Row> selectMatchingRowsWithoutIndex(SQLTerm[] arrSQLTerms, String[] strarrOperators)
			throws DBAppException, IOException {
		String tableName = arrSQLTerms[0]._strTableName;
		return selectMatchingRows(arrSQLTerms, strarrOperators, getTablePagesIDs(tableName));
	}

	private Set<Row> selectMatchingRowsUsingIndex(SQLTerm[] arrSQLTerms, String[] strarrOperators)
			throws DBAppException, IOException {
		...
		while (termsList.size() > 1) {
			if (termsList.size() >= 3 && termsFormIndex(termsList.get(0), termsList.get(1), termsList.get(2), tableName)
					&& operatorsList.get(0).equals("AND") && operatorsList.get(1).equals("AND")) {
				...
				Set<Row> rowsFromIndex = selectMatchingRows(new SQLTerm[] { term1, term2, term3 },
						new String[] { operator1, operator2 }, pagesIDsToSearchInto);
	...
	private Set<Row> selectMatchingRows(SQLTerm[] arrSQLTerms, String[] strarrOperators,
			Set<Integer> pagesIDsToSearchInto) throws DBAppException, IOException {
		...
		for (int i = 0; i < arrSQLTerms.length; i++) {
			for (Integer page_id : pagesIDsToSearchInto) {
				String pagePath = "src/resources/tables/" + arrSQLTerms[0]._strTableName + "/pages/page" + page_id
						+ ".ser";
				Page page = (Page) deserialize(pagePath);
	...
	private Set<Row> singleSQLTermResult(SQLTerm sqlTerm, Page page) throws IOException, DBAppException {
		...
		for (Row row : page.getData()) {
			...
			case "=":
				if (cellValue.compareTo(sqlTermValue) == 0)
					result.add(row);
				break;
			case "!=":
				if (cellValue.compareTo(sqlTermValue) != 0)
	...
	private <T> Set<T> applyOperatorsFromLeftToRight(Set<T>[] separated_SQLTermsResults, String[] strarrOperators)
			throws DBAppException { // to do
	...
	public Set<Integer> getPagesFromIndex(SQLTerm sqlTerm1, SQLTerm sqlTerm2, SQLTerm sqlTerm3, String tableName)
		...
		for (String currentIndex : allIndicies) {
			if (found)
				break;
			for (String possibleName : Methods2.getAllPermutations(sqlTerm1._strColumnName, sqlTerm2._strColumnName,
					sqlTerm3._strColumnName)) {
		...
		Methods2.fillSetWithPages_satisfyingCondition_forInputValue(octree, resultPages, x_dimention_term._strOperator,
				y_dimention_term._strOperator, z_dimention_term._strOperator, (Comparable) x_dimention_term._objValue,
				(Comparable) y_dimention_term._objValue, (Comparable) z_dimention_term._objValue);
		serialize(octreePath, octree);
		return resultPages;
	}

	public boolean indexCanHelp_simplified(SQLTerm[] arrSQLTerms, String[] strarrOperators)
			throws DBAppException, IOException {
	...
	public boolean termsFormIndex(SQLTerm term1, SQLTerm term2, SQLTerm term3, String tableName) throws IOException {
		for (String indexName : getAllTableIndicies(tableName))
			for (String posiibleIndexName : Methods2.getAllPermutations(term1._strColumnName, term2._strColumnName,
					term3._strColumnName))
	...
	private void validateColumnExistsInTable(String strColName, String strTableName)
			throws DBAppException, IOException {
		validateTableExists(strTableName);
		boolean found = false;
		BufferedReader br = new BufferedReader(new FileReader("MetaData.csv"));
	...
	private void validateSQLTerms(SQLTerm[] arrSQLTerms) throws DBAppException, IOException {
		...
		for (SQLTerm term : arrSQLTerms) {
			...
			boolean operatorIsSupported = operator.equals("=") || operator.equals("!=") || operator.equals(">")
